...
- Default NVIDIA lane: vision and summarization both run the native CUDA `llama-server` built at pinned llama.cpp `b7782` (matching the ROCm lane). Vision serves the largest downloaded Q4 candidate that fits the selected GPU via `VISION_MODEL_CANDIDATES_JSON`; endpoints sharing a card that cannot hold both swap per stage with a short idle linger (`SWAP_IDLE_TIMEOUT_SECONDS`). The official `vLLM` image remains available through the `vision-vllm` compose profile for large-VRAM hosts (for example Blackwell-class cards).
