...
def test_rocm_llama_server_files_fail_closed_without_rocm():
    dockerfile_text = (ROOT / "docker/rocm-llama-server/Dockerfile").read_text(encoding="utf-8")
    entrypoint_text = (ROOT / "docker/rocm-llama-server/entrypoint.sh").read_text(encoding="utf-8")
